...
namespace TAMU.GeoInnovation.PointIntersectors.Census.MySql.Census2010
{

    [Serializable]
    public class MySqlCensus2010PointIntersector : AbstractCensus2010PointIntersector
    {
        ...
        public MySqlCensus2010PointIntersector()
            : base()
        { }

        public MySqlCensus2010PointIntersector(double version, IQueryManager blockFilesQueryManager, IQueryManager stateFilesQueryManager, IQueryManager countryFilesQueryManager)
        ...
        public override string GetStateFips(double longitude, double latitude)
        {
            ...
            try
            {
                ...
                if ((latitude <= 90 && latitude >= -90) && (longitude <= 180 && longitude >= -180))
                {
                    ...
                    sql += "  Contains(shapeGeom, GeomFromText('POINT(" + latitude + " " + longitude + ")')) = 1";
            ...
            try
            {
                if ((latitude <= 90 && latitude >= -90) && (longitude <= 180 && longitude >= -180))
                {
                    if (StateUtils.isState(state))
                    {
                        ...
                        sql += " WITH (INDEX (idx_geog))";
                        sql += " WHERE ";
                        sql += " st_distance(shapeGeom, GeomFromText('POINT(?latitude ?longitude)')) <= ?distanceThreshold ";
